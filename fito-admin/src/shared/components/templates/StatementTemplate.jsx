...
const StatementTemplate = ({ data, onNext }) => {
  ...
  }, [text, animationType, typingSpeed]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 relative overflow-hidden" style={{ backgroundColor }}>
      <motion.div 
      ...
      <div className="relative z-10 w-full max-w-4xl text-center">
        {animationType === 'typewriter' ? (
          <div style={{ fontSize, lineHeight: 1.4, fontWeight: 600, color: textColor, fontFamily: 'Georgia, serif', whiteSpace: 'pre-wrap', textShadow: `0 0 10px ${accentColor}1A` }}>
            {displayedText}
            <motion.span animate={{ opacity: [1, 0, 1] }} transition={{ repeat: Infinity, duration: 0.8, ease: "steps(2)" }} className="inline-block w-3 h-[1em] ml-1 align-middle" style={{ backgroundColor: accentColor }} />
          </div>
        ) : (
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-wrap justify-center items-center" style={{ fontSize, lineHeight: 1.4, fontWeight: 600, color: textColor, fontFamily: '"Inter", sans-serif', textShadow: `0 0 20px ${accentColor}22` }}>
            {characters.map((char, index) => (
              <motion.span key={`${char}-${index}`} variants={childVariants} custom={animationType} style={{ display: char === ' ' ? 'inline' : 'inline-block', whiteSpace: 'pre' }}>
                {char}
              </motion.span>
            ))}
          </motion.div>
        ...
        <AnimatePresence>
          {isComplete && (
            <motion.button initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} whileTap={{ scale: 0.95 }} onClick={onNext} className="mt-20 group relative overflow-hidden">
